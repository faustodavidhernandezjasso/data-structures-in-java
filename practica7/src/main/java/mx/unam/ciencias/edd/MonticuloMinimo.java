...
public class MonticuloMinimo<T extends ComparableIndexable<T>>
   ...
   /**
    ...
    private void acomodandoHaciaAbajo(T elemento) {
        ...
        if (validaIndice(hi)) {
            if (validaIndice(hd)) {
                minimo = this.arbol[hi].compareTo(this.arbol[hd]) < 0 ? hi : hd;
            }
        } else {
            minimo = hd;
        }
        if (this.arbol[indice].compareTo(this.arbol[minimo]) <= 0) {
            return;
        } else {
            intercambiaElementos(indice, minimo);
            acomodandoHaciaAbajo(this.arbol[minimo]);
        }
    }
    */
